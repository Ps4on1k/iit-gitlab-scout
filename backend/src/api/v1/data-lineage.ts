...
const lineageData: {
  ...
  collectors: {
    ...
    "branch-collector": { writes_to: ["project_branches", "commits"], description: "Сбор веток и их метаданных" },
    ...
    "activity-collector": { writes_to: ["project_activity"], description: "Агрегация дневной активности (коммиты, MR, пайплайны)" },
    "dependency-audit": { writes_to: ["project_dependencies_audit"], description: "Аудит зависимостей и проверка актуальности версий" },
    "contributor-sync": { writes_to: ["contributor_directory"], description: "Автообнаружение групп контрибьюторов и синхронизация справочника" },
    "clickhouse-sync": { writes_to: ["ClickHouse"], description: "Синхронизация данных из PostgreSQL в ClickHouse для OLAP" },
  },
  staging: {
  ...
  api_endpoints: {
    "/api/v1/dashboard": { reads_from: ["commits", "project_branches", "project_merge_requests", "project_deployments", "contributor_directory", "mart_activity", "mart_dashboard"], description: "Главная панель обзора — KPI, DORA, активность" },
    "/api/v1/contributor-analytics": { reads_from: ["contributor_profiles", "contributor_directory", "commits"], description: "Аналитика контрибьюторов — метрики, heatmap, скоринг" },
    "/api/v1/mr-analytics": { reads_from: ["project_merge_requests", "contributor_directory"], description: "Аналитика merge request'ов" },
    "/api/v1/pipelines": { reads_from: ["project_pipelines", "projects"], description: "Аналитика пайплайнов CI/CD" },
    "/api/v1/dora-metrics": { reads_from: ["project_deployments"], description: "DORA-метрики: частота деплоев, lead time, MTTR" },
    "/api/v1/red-flags": { reads_from: ["commits", "project_merge_requests", "project_pipelines", "project_deployments", "project_branches"], description: "Красные флаги: ночь, bus factor, churn, weekend" },
    "/api/v1/benchmark": { reads_from: ["mart_benchmark"], description: "Бенчмарк по тегам" },
    "/api/v1/branches": { reads_from: ["project_branches", "projects"], description: "Список веток с метриками здоровья" },
    "/api/v1/issues": { reads_from: ["project_issues", "projects"], description: "Аналитика задач (issues)" },
    ...
    "/api/v1/dependencies": { reads_from: ["project_dependencies_audit", "projects"], description: "Аудит зависимостей" },
    "/api/v1/contributor-directory": { reads_from: ["contributor_directory"], description: "CRUD справочника контрибьюторов" },
    "/api/v1/executive-report": { reads_from: ["mart_executive_report"], description: "Исполнительный отчёт" },
    "/api/v1/data-lineage": { reads_from: ["lineage_metadata"], description: "Потоки данных: граф, статистика" },
  },
};
