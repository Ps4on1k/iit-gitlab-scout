-- Materialized view for benchmark data
...
tag_commits as (
  ...
  join {{ ref('stg_commits') }} c on c.project_id = tp.project_id
...
tag_deploys as (
  ...
  from tag_projects tp
  join {{ ref('stg_deployments') }} d on d.project_id = tp.project_id
...
tag_pipelines as (
  ...
  from tag_projects tp
  join {{ ref('stg_pipelines') }} p on p.project_id = tp.project_id
...
tag_mr as (
  ...
  from tag_projects tp
  join {{ ref('stg_merge_requests') }} mr on mr.project_id = tp.project_id
